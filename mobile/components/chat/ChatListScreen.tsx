...
/**
 * The conversation list. Rendered in two places:
 *  - `variant="tab"` — the Chats tab, which has no back stack.
 *  - `variant="stack"` — the /messages route, kept so existing deep links and
 *    the "no back stack, go somewhere sane" fallbacks still resolve.
 *
 * The only behavioural difference is the header's back button.
 */
export default function ChatListScreen({
  ...
  // The Chats TAB is reachable while logged out (the tab layout admits guests),
  // where every chat call would 401. Resolved on mount; null means "not checked
  // yet", so we don't flash the gate before we know.
  ...
  if (isGuest) {
    return (
      <GuestGate
        ...
        subtitle="Create an account to chat with hosts, vendors and people you follow."
      />
    );
  }
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            {/* A tab has no back stack — only the /messages stack route needs this. */}
            {variant === "stack" && <GlassBackButton onPress={() => goHome()} size={36} />}
            <View>
